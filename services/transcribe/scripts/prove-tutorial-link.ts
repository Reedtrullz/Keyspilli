/**
 * Private feasibility runner: original YouTube URL -> tutorial search -> MIDI.
 * Not a production route or musical-identity/rights certificate.
 * Run from repo root with Node22: npx tsx services/transcribe/scripts/prove-tutorial-link.ts URL NEW_OUTPUT_DIR
 * Requires the separately installed output/tutorial-recovery/venv Python runtime.
 */
...
const url=normalizeYoutubeImportUrl(process.argv[2]);
...
for(const c of candidates){
 ...
 try{
  ...
  await call(resolve('output/tutorial-recovery/venv/bin/python'),[resolve('services/transcribe/src/tutorial_keys.py'),join(dir,'video.mp4'),'--output',join(dir,'extracted.json')],600000);
