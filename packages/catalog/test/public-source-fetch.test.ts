...
describe("automatic source network boundary", () => {
  it.each(["100.64.0.1", "198.18.0.1", "224.0.0.1", "ff02::1", "fec0::1"])("rejects non-public resolved address %s", async (address) => {
    lookup.mockResolvedValue([{ address, family: address.includes(":") ? 6 : 4 }]);
    const result = await retrieveExternalSource({ initialUrl: "https://scores.example/song.mid" }, { allowNetwork: true });
    expect(JSON.stringify(result)).toMatch(/public addresses/);
  });
