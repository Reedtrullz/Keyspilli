...
worker off for MIDI, MusicXML, and MXL; the live deployment kept the existing
worker image unchanged and running. A deterministic live MIDI upload exercised
...
## 2026-09-05 — Rights-cleared dense-metal AMT evaluation corpus

The bounded official-source survey found no real dense-metal dataset with
complete aligned pitched-note truth. HiMMP/In Solitude is usable as
`REAL_DOMAIN_PARTIAL_REFERENCE`: its CC BY reference mix, native kick MIDI,
tempo map, and orchestration MIDI are reproducibly available, but guitar and
bass note truth are absent. The frozen 252–282 second HiMMP window therefore
permits kick-onset scoring only; it must never be reported as full
transcription accuracy. The external-corpus decision is
`REAL_PARTIAL_REFERENCE_METAL_CORPUS_FOUND`.

The full-reference fallback is three original, project-owned 32-second pieces:
`METAL_A_TIGHT_RIFF`, `METAL_B_DENSE_EXTREME`, and
`METAL_C_LAYERED_MELODIC`. They cover tight repeated riffing, high-density
rapid material, and melodic/harmonic layering with four or five explicit
roles. Canonical event JSON, role-aware MIDI including channel-10 drums, and
one stereo PCM16 full mix per piece are generated by the deterministic
`keyspilli-procedural-metal-v1` renderer. It uses no samples, SoundFonts,
plugins, models, or third-party audio assets. Two 44.1 kHz builds were byte
identical; all MIDI and WAV files validated, and the frozen duration is 96
seconds total.

Decision: `RIGHTS_CLEARED_REAL_PLUS_SYNTHETIC_METAL_CORPUS_READY`. The corpus
is `EVAL_ONLY`: it cannot enter generation, training, tuning, or policy
selection. MuScriptor was **not run** on any frozen piece, so
`MUSCRIPTOR_RAW_METAL_HEADROOM_UNEVALUATED` remains unchanged. Independent
score/audio alignment remains `REAL_SYMBOLIC_ALIGNMENT_PARTIAL`; musical
quality remains `MUSICAL_QUALITY_NOT_OBJECTIVELY_ESTABLISHED`; human listening
