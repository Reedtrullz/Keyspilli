...
T4 evidence: `a8943bb`, `0a37afd`, the current runtime candidate `fce18bcbc358a06281813192de95eb56f9f0e878`, `packages/midi/test/midi.test.ts` (`records the real notes.json chord fallback origin before player melody selection` plus the negative LH-selected fixture), `packages/player-core/test/melody-accompaniment.test.ts`, and `apps/web/src/components/player/chord-sources.test.ts`. The actual fallback origin is `packages/catalog/src/ingest.ts` → `packages/midi/src/simplify.ts:buildVariants/chordsAt` → `notes.json.chords` → `packages/catalog/src/chord-timeline.ts:generatedTimeline` → `catalog-api` → `Player`. The mixed LH fixture excludes its upper selected voice, while the negative LH-selected/no-clean-cluster fixture shows that `chordsAt` can include the selected pitch. `chordsAt` has no selected-melody/source-note identity parameter and the historical `notes.json` boundary carries no protected source ID/source lane. Runtime now has selected indices/IDs after T3 and the new candidate applies `none` / `authored-only` / `all` by selected source origin: generated notes-derived harmony stays label-only, Auto preserves explicit authored chart support, and explicit UG chart events may generate support. A selected-R/low-voicing fixture proves the old sounding guard could allow non-colliding generated support; the new policy emits zero generated support and an honest `unverified chord source` fallback. No general source-data exclusion or musical harmony claim is made.
