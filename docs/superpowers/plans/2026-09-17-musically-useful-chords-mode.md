...
- [x] Choose and implement the runtime-only policy for unverified generated harmony. The new candidate treats `generated` notes-derived events as display-only for Melody + accompaniment, allows only `sourceKind: "authored"` events in the Auto hybrid, and retains harmonic generation for the explicit UG chart source. Existing selected-index source reduction/Original fallback remains the support path; Bass + chords is unchanged. Protected-note re-inference through `simplifyPianoAccompaniment({ protectedNotes })` remains a larger audible candidate requiring a semantic-to-timed-chord adapter. The candidate uses development fixtures only; the frozen reserved result is not altered.
...
T4 evidence: `a8943bb`, `0a37afd`, the runtime-policy commit `7775d5b383289a28212dc9c2a8ec2e1f3b752d81`, the status-accounting commit `f0cb469`, the frozen-fixture comparison in `docs/superpowers/evidence/2026-09-17-chords-v2-current-candidate-development.json`, `packages/midi/test/midi.test.ts` (`records the real notes.json chord fallback origin before player melody selection` plus the negative LH-selected fixture), `packages/player-core/test/melody-accompaniment.test.ts`, and `apps/web/src/components/player/chord-sources.test.ts`. The `fce18bc` run is retained as the `harmonicSupport: "all"` comparison baseline. The actual fallback origin is `packages/catalog/src/ingest.ts` → `packages/midi/src/simplify.ts:buildVariants/chordsAt` → `notes.json.chords` → `packages/catalog/src/chord-timeline.ts:generatedTimeline` → `catalog-api` → `Player`. The mixed LH fixture excludes its upper selected voice, while the negative LH-selected/no-clean-cluster fixture shows that `chordsAt` can include the selected pitch. `chordsAt` has no selected-melody/source-note identity parameter and the historical `notes.json` boundary carries no protected source ID/source lane. Runtime now has selected indices/IDs after T3 and the candidate applies `none` / `authored-only` / `all` by selected source origin: generated notes-derived harmony stays label-only, Auto preserves explicit authored chart support, and explicit UG chart events may generate support. A selected-R/low-voicing fixture proves the old sounding guard could allow non-colliding generated support; the new policy emits zero generated support, keeps successful source-reduction intervals informational, and reports an honest `unverified chord source` fallback only where blocked backing has no source replacement. The complete current Blackbird/Oops/Hell impact and current Oops `[64,108]` browser capture are recorded as current candidate evidence; no general source-data exclusion or musical harmony claim is made.
