...
The parent accepted `de9cfd0` as a partial density/silence fix, not as T5 completion. Commit `55690fb` continues that checkpoint with exact synthetic fixtures for syncopated/pickup source attacks, repeated protected hooks, redundant source stacks, repeated chart chords, off-grid changes, 3/4-like, 6/8-like and unknown-meter grids. Commit `ca839b0` adds the Player meter handoff and interval-local sounding repair. Local partial-chart gaps and one uncertain no-chart interval now reduce independently; a single global unresolved span no longer disables unrelated no-chart intervals. Exact repeated source stacks are reduced after their first attack, while broader motif protection still depends on explicit phrase selection.

The sparse harmonic candidate now retains each supported adjacent harmonic boundary and, when the Player supplies a validated time signature, adds at most two phase-locked attacks per complete measure. Unknown meter remains boundary/source-timed. Adjacent repeated chart chords do not receive an unconditional quarter-note pattern, and off-grid chord changes remain off-grid. `generatedNoteCount` and `generatedBeats` are measured from rendered events after sounding-limit filtering.
...
The current producer was run over the complete frozen Blackbird, Oops, and Hell development fixtures with their Player-equivalent normalized durations, automatic selection, and captured time signatures. These are exact source/output accounting and phrase-status comparisons, not listening results:
...
| Oops | 1,891 → 1,388 | 109.758 / 170.375 / 55.867 | 44.375 | 420; 276 | 284 / 16 / 113 / 7 |
| Hell | 1,130 → 1,108 | 22.875 / 291.875 / 37.250 | 22.500 | 222; 66 | 153 / 2 / 65 / 2 |

The output also retained explicit fallback/unresolved provenance: Blackbird 90 fallback spans and 20 unresolved spans; Oops 284 fallback spans and 54 unresolved spans; Hell 175 fallback spans and 27 unresolved spans. The increased structural reduction and sparse backing counts are not a success score; no automatic melody gold, harmonic review, playability review, or human musical acceptance exists for these fixtures.

## T6 sounding-limit checkpoint

The current T6 work in `ca839b0` adds a shared post-projection sounding pass. It counts melody and retained-unclassified events in each hand budget, preserves them as mandatory, and trims accompaniment only over rejected intervals before exceeding three simultaneously sounding notes per physical hand or a 12-semitone active span. Surviving pieces retain their `sourceNoteIds`; same-pitch melody/support collisions are removed even across hand labels; late collisions no longer erase earlier held support. It records `sounding limit exceeded` over the rejected interval and reports source/generated counts after trimming. Synthetic tests cover held overlap, same-pitch collision, retained-unclassified overlap, late collision, and all supported meter-phase candidates; the full voicing, pedal, low-register, and no-feasible-allocation matrix remains open.

Verification for the continued T5/T6 checkpoint: focused player-core `47/47`, full player-core `15 files / 219 tests PASS`, focused MIDI `120/120`, full MIDI `17 files / 396 tests PASS`, focused catalog piano-section-builder `9/9`, full catalog `120 files / 1,102 tests PASS`, focused web SoundControls `2/2`, full web `35 files / 202 tests PASS`, and all workspace typechecks pass. These are synthetic and structural checks. They do not establish syncopation, pickup, 3/4, 6/8, total sounding playability, recognizability, harmonic plausibility, UI status correctness, real-song usefulness, or musical acceptance.
