...
Status: T1 source capture accepted by parent; G1 amendments are recorded below. T2 accounting may proceed independently; T3 melody tuning remains gated.
...
## Proposed melody/rest expectations

[2026-09-17-oops-proposed-melody-expectations.json](./2026-09-17-oops-proposed-melody-expectations.json) now contains concrete source-ID hypotheses for every development window. The intro control proposes an explicit empty melody (`[]`) across `[0,16]` because all 24 source events are L-labelled. The `[16,32]` window proposes four exact R-labelled re-entry IDs at beats 16 and 24, with rests `[18,24]` and `[25.875,32]`. Sections 2, 3, and 4 contain exact candidate R IDs with duration `>=0.5` beats and no asserted rest because the source is R-dense and lacks source-lane truth; the ambiguity window is explicitly “uncertain active” and does not force either a rest or a melody label.

These are review hypotheses derived from source hand/duration/onset evidence, not labels generated by or certified against the current selector. The live payload cannot recover `sourceLane` or `identitySource`; original-source inspection/re-import is the separate recovery prerequisite if these hypotheses remain unresolved.
...
|---|---|---|---|---|
| T1-F01 | High | Amended | The 223 removed Oops events are expected under the current one-melody-note-per-onset/source-support reduction and are not proof of a bad melody. Separately, the existing heuristic emits 30 ambiguity spans including the reported 83.5–89.5 region; it has no rest state or source-lane truth. | T2 must separate intentional reduction accounting from T3 rest/re-entry and selected-path uncertainty; musical usefulness remains unjudged |
