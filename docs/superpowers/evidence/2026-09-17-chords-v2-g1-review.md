...
## Historical pre-label-only development-fixture structural comparison

The following comparison predates the label-only candidate and is retained as historical evidence. It was run over the complete frozen Blackbird, Oops, and Hell development fixtures with Player-equivalent normalized durations and automatic selection. Because the Player has no validated phase provenance, that run intentionally omitted meter-phase timing and exercised the boundary/source fallback. These are exact source/output accounting and phrase-status comparisons, not listening results; they are not current counts for the new candidate.
