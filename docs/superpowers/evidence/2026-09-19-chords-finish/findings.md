...
| F8 | Source identity/rest evidence is incomplete for automatic melody promotion. | P1 | T1/T3 | Full-source inspection, bounded source recovery if justified, no hand-label shortcut, and two-failed-iteration source investigation rule. | Open at T1. Queen’s current importer replay uniquely traces all 278 raw CANTO roots: 129 are rejected before Advanced output and 38 undergo non-grid transforms, but no semantic role is promoted. Oops backup staff/voice is recoverable but has no semantic legend; Blackbird worksheet remains reviewer-owned; Somebody/Your Song/Hell have no runtime semantic melody contract. |
