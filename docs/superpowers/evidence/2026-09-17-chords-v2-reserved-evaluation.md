...
For retrospective deterministic coverage only, the same frozen producer is also evaluated on these non-overlapping windows selected after the primary freeze: Near the Cross `0–24` beats, Prélude `0–16`, Pay Me My Money Down `0–16`, and Dear God `64–96`. Their `selectionBasis` is recorded as `retrospective non-overlapping control window selected after primary freeze; not an untouched holdout`. These windows add no audio captures and no human ratings; they are not the requested two useful phrases per song.
